 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradingform_matrix_controller extends gradingform_controller {
    // Modes of displaying the rubric (used in gradingform_matrix_renderer)
    ...
    public function extend_settings_navigation(settings_navigation $settingsnav, ?navigation_node $node=null) {
        $node->add(get_string('definerubric', 'gradingform_matrix'),
            $this->get_editor_url(), settings_navigation::TYPE_CUSTOM,
            null, null, new pix_icon('icon', '', 'gradingform_matrix'));
    }
    ...
    public function extend_navigation(global_navigation $navigation, ?navigation_node $node=null) {
        ...
        if ($this->is_form_defined() && ($options = $this->get_options()) && !empty($options['alwaysshowdefinition'])) {
            $node->add(get_string('gradingof', 'gradingform_matrix', get_grading_manager($this->get_areaid())->get_area_title()),
     ...
     * @param stdClass $newdefinition rubric definition data as coming from gradingform_matrix_editrubric::get_data()
     * @param int|null $usermodified optional userid of the author of the definition, defaults to the current user
     */
    public function update_definition(stdClass $newdefinition, $usermodified = null) {
        $this->update_or_check_matrix($newdefinition, $usermodified, true);
        if (isset($newdefinition->matrix['regrade']) && $newdefinition->matrix['regrade']) {
     ...
     *
     */
    public function update_or_check_matrix(stdClass $newdefinition, $usermodified = null, $doupdate = false) {
        global $DB;
        ...
        if (!isset($newdefinition->matrix['options'])) {
            $newdefinition->matrix['options'] = self::get_default_options();
        }
        $newdefinition->options = json_encode($newdefinition->matrix['options']);
        ...
        $newlockzeropoints = $newdefinition->matrix['options']['lockzeropoints'];
        ...
        // update rubric data
        if (empty($newdefinition->matrix['criteria'])) {
            $newcriteria = array();
        } else {
            $newcriteria = $newdefinition->matrix['criteria']; // new ones to be saved
        }
        $currentcriteria = $currentdefinition->matrix_criteria;
        ...
        foreach ($newcriteria as $id => $criterion) {
            ...
            if (preg_match('/^NEWID\d+$/', $id)) {
                ...
                if ($doupdate) {
                    $id = $DB->insert_record('gradingform_matrix_criteria', $data);
                }
                $haschanges[5] = true;
            } else {
                ...
                if (!empty($data)) {
                    ...
                    $data['id'] = $id;
                    if ($doupdate) {
                        $DB->update_record('gradingform_matrix_criteria', $data);
                    }
                    $haschanges[1] = true;
                }
                ...
                foreach ($currentcriteria[$id]['levels'] as $levelid => $currentlevel) {
                    ...
                    if (!array_key_exists($levelid, $levelsdata)) {
                        if ($doupdate) {
                            $DB->delete_records('gradingform_matrix_levels', array('id' => $levelid));
                        }
                        $haschanges[4] = true;
                    }
                }
            ...
            foreach ($levelsdata as $levelid => $level) {
                ...
                if (preg_match('/^NEWID\d+$/', $levelid)) {
                    ...
                    if ($doupdate) {
                        $levelid = $DB->insert_record('gradingform_matrix_levels', $data);
                    }
                    ...
                    if (!empty($data)) {
                        ...
                        $data['id'] = $levelid;
                        if ($doupdate) {
                            $DB->update_record('gradingform_matrix_levels', $data);
                        }
                        if (isset($data['score'])) {
                            $haschanges[3] = true;
                        }
        ...
        foreach (array_keys($currentcriteria) as $id) {
            if (!array_key_exists($id, $newcriteria)) {
                if ($doupdate) {
                    $DB->delete_records('gradingform_matrix_criteria', array('id' => $id));
                    $DB->delete_records('gradingform_matrix_levels', array('criterionid' => $id));
                }
                $haschanges[3] = true;
            }
        }
     ...
     *
     * There is a new array called 'matrix_criteria' appended to the list of parent's definition properties.
     */
    protected function load_definition() {
        ...
        $sql = "SELECT gd.*,
             ...
             LEFT JOIN {gradingform_matrix_criteria} rc ON (rc.definitionid = gd.id)
             LEFT JOIN {gradingform_matrix_levels} rl ON (rl.criterionid = rc.id)
        ...
        foreach ($rs as $record) {
            ...
            if ($this->definition === false) {
                ...
                $this->definition->matrix_criteria = array();
            }
            // pick the criterion data
            if (!empty($record->rcid) and empty($this->definition->matrix_criteria[$record->rcid])) {
                foreach (array('id', 'sortorder', 'description', 'descriptionformat') as $fieldname) {
                    $this->definition->matrix_criteria[$record->rcid][$fieldname] = $record->{'rc'.$fieldname};
                }
                $this->definition->matrix_criteria[$record->rcid]['levels'] = array();
            }
            // pick the level data
            if (!empty($record->rlid)) {
                foreach (array('id', 'score', 'definition', 'definitionformat') as $fieldname) {
                    ...
                    $this->definition->matrix_criteria[$record->rcid]['levels'][$record->rlid][$fieldname] = $value;
                }
            }
        }
        $rs->close();
        ...
        if (!$options['sortlevelsasc']) {
            foreach (array_keys($this->definition->matrix_criteria) as $rcid) {
                $this->definition->matrix_criteria[$rcid]['levels'] = array_reverse($this->definition->matrix_criteria[$rcid]['levels'], true);
            }
        }
    }
    ...
    public function get_definition_for_editing($addemptycriterion = false) {
        ...
        $properties->matrix = array('criteria' => array(), 'options' => $this->get_options());
        if (!empty($definition->matrix_criteria)) {
            $properties->matrix['criteria'] = $definition->matrix_criteria;
        } else if (!$definition && $addemptycriterion) {
            $properties->matrix['criteria'] = array('addcriterion' => 1);
        }
    ...
    public function get_definition_copy(gradingform_controller $target) {
        ...
        $new->matrix = array('criteria' => array(), 'options' => $old->matrix['options']);
        $newcritid = 1;
        $newlevid = 1;
        foreach ($old->matrix['criteria'] as $oldcritid => $oldcrit) {
            unset($oldcrit['id']);
            ...
            $new->matrix['criteria']['NEWID'.$newcritid] = $oldcrit;
            $newcritid++;
        }
     ...
     * @return gradingform_matrix_renderer
     */
    ...
    public function render_preview(moodle_page $page) {
        ...
        $criteria = $this->definition->matrix_criteria;
        $options = $this->get_options();
        $rubric = '';
        ...
        if ($showdescription) {
            $rubric .= $output->box($this->get_formatted_description(), 'gradingform_matrix-description');
        }
        if (has_capability('moodle/grade:managegradingforms', $page->context)) {
            if (!$options['lockzeropoints']) {
                // Warn about using grade calculation method where minimum number of points is flexible.
                $rubric .= $output->display_matrix_mapping_explained($this->get_min_max_score());
            }
            $rubric .= $output->display_matrix($criteria, $options, self::DISPLAY_PREVIEW, 'matrix');
        } else {
            $rubric .= $output->display_matrix($criteria, $options, self::DISPLAY_PREVIEW_GRADED, 'matrix');
        }
    ...
    protected function delete_plugin_definition() {
        ...
        // delete all fillings
        $DB->delete_records_list('gradingform_matrix_fillings', 'instanceid', $instances);
        // delete instances
        $DB->delete_records_list('grading_instances', 'id', $instances);
        // get the list of criteria records
        $criteria = array_keys($DB->get_records('gradingform_matrix_criteria', array('definitionid' => $this->definition->id), '', 'id'));
        // delete levels
        $DB->delete_records_list('gradingform_matrix_levels', 'criterionid', $criteria);
        // delete critera
        $DB->delete_records_list('gradingform_matrix_criteria', 'id', $criteria);
    }
    ...
    public function get_or_create_instance($instanceid, $raterid, $itemid) {
        ...
        if ($itemid && $raterid) {
            ...
            if ($rs = $DB->get_records('grading_instances', $params, 'timemodified DESC', '*', 0, 1)) {
                ...
                if ($record->status == gradingform_matrix_instance::INSTANCE_STATUS_INCOMPLETE &&
    ...
    public static function sql_search_from_tables($gdid) {
        return " LEFT JOIN {gradingform_matrix_criteria} rc ON (rc.definitionid = $gdid)
                 LEFT JOIN {gradingform_matrix_levels} rl ON (rl.criterionid = rc.id)";
    }
    ...
    public function get_min_max_score() {
        ...
        foreach ($this->get_definition()->matrix_criteria as $id => $criterion) {
            $scores = array();
    ...
    /**
     * @return array An array containing a single key/value pair with the 'matrix_criteria' external_multiple_structure.
     * @see gradingform_controller::get_external_definition_details()
     * @since Moodle 2.5
     */
    public static function get_external_definition_details() {
        $matrix_criteria = new external_multiple_structure(
        ...
        return array('matrix_criteria' => $matrix_criteria);
    }
...
class gradingform_matrix_instance extends gradingform_instance {
    ...
    public function cancel() {
        global $DB;
        parent::cancel();
        $DB->delete_records('gradingform_matrix_fillings', array('instanceid' => $this->get_id()));
    }
    ...
    public function copy($raterid, $itemid) {
        ...
        $currentgrade = $this->get_matrix_filling();
        foreach ($currentgrade['criteria'] as $criterionid => $record) {
            ...
            $DB->insert_record('gradingform_matrix_fillings', $params);
        }
        return $instanceid;
    }
    ...
    public function is_empty_form($elementvalue) {
        $criteria = $this->get_controller()->get_definition()->matrix_criteria;
    ...
    public function clear_attempt($data) {
        ...
        foreach ($data['criteria'] as $criterionid => $record) {
            $DB->delete_records('gradingform_matrix_fillings',
    ...
    public function update($data) {
        ...
        foreach ($data['criteria'] as $criterionid => $record) {
            ...
            if (!array_key_exists($criterionid, $currentgrade['criteria'])) {
                ...
                $DB->insert_record('gradingform_matrix_fillings', $newrecord);
            } else {
                ...
                if (count($newrecord) > 1) {
                    $DB->update_record('gradingform_matrix_fillings', $newrecord);
                }
            }
        }
        foreach ($currentgrade['criteria'] as $criterionid => $record) {
            if (!array_key_exists($criterionid, $data['criteria'])) {
                $DB->delete_records('gradingform_matrix_fillings', array('id' => $record['id']));
            }
        }
        $this->get_matrix_filling(true);
    }
    ...
    public function get_grade() {
        ...
        foreach ($grade['criteria'] as $id => $record) {
            $curscore += $this->get_controller()->get_definition()->matrix_criteria[$id]['levels'][$record['levelid']]['score'];
        }
    ...
    public function render_grading_element($page, $gradingformelement) {
        ...
        if (!$gradingformelement->_flagFrozen) {
            ...
            if ($gradingformelement->_persistantFreeze) {
                $mode = gradingform_matrix_controller::DISPLAY_EVAL_FROZEN;
            } else {
        ...
        $html = '';
        if ($value === null) {
            $value = $this->get_matrix_filling();
        } else if (!$this->validate_grading_element($value)) {
            $html .= html_writer::tag('div', get_string('rubricnotcompleted', 'gradingform_matrix'), array('class' => 'gradingform_matrix-error'));
        }
        $currentinstance = $this->get_current_instance();
        if ($currentinstance && $currentinstance->get_status() == gradingform_instance::INSTANCE_STATUS_NEEDUPDATE) {
            $html .= html_writer::div(get_string('needregrademessage', 'gradingform_matrix'), 'gradingform_matrix-regrade',
                                      array('role' => 'alert'));
        }
        $haschanges = false;
        if ($currentinstance) {
            $curfilling = $currentinstance->get_matrix_filling();
        ...
        if ($this->get_data('isrestored') && $haschanges) {
            $html .= html_writer::tag('div', get_string('restoredfromdraft', 'gradingform_matrix'), array('class' => 'gradingform_matrix-restored'));
        }
        if (!empty($options['showdescriptionteacher'])) {
            $html .= html_writer::tag('div', $this->get_controller()->get_formatted_description(), array('class' => 'gradingform_matrix-description'));
        }
        $html .= $this->get_controller()->get_renderer($page)->display_matrix($criteria, $options, $mode, $gradingformelement->getName(), $value);
        return $html;
    }
}
