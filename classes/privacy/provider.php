...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('gradingform_matrix_fillings', [
    ...
    public static function export_gradingform_instance_data(\context $context, int $instanceid, array $subcontext) {
        ...
        $sql = "SELECT rc.description, rl.definition, rl.score, rf.remark
                  ...
                  JOIN {gradingform_matrix_criteria} rc ON rc.id = rf.criterionid
                  JOIN {gradingform_matrix_levels} rl ON rf.levelid = rl.id
                 WHERE rf.instanceid = :instanceid";
        $records = $DB->get_records_sql($sql, $params);
        if ($records) {
            $subcontext = array_merge($subcontext, [get_string('rubric', 'gradingform_matrix'), $instanceid]);
    ...
    public static function delete_gradingform_for_instances(array $instanceids) {
        global $DB;
        $DB->delete_records_list('gradingform_matrix_fillings', 'instanceid', $instanceids);
    }
}
