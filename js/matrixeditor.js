M.gradingform_matrixeditor = {'templates' : {}, 'eventhandler' : null, 'name' : null, 'Y' : null};

/**
 * This function is called for each matrixeditor on page.
 */
M.gradingform_matrixeditor.init = function(Y, options) {
    M.gradingform_matrixeditor.name = options.name
    M.gradingform_matrixeditor.Y = Y
    M.gradingform_matrixeditor.templates[options.name] = {
        'criterion' : options.criteriontemplate,
        'level' : options.leveltemplate
    }
    M.gradingform_matrixeditor.disablealleditors()
    Y.on('click', M.gradingform_matrixeditor.clickanywhere, 'body', null)
    YUI().use('event-touch', function (Y) {
        Y.one('body').on('touchstart', M.gradingform_matrixeditor.clickanywhere);
        Y.one('body').on('touchend', M.gradingform_matrixeditor.clickanywhere);
    })
    M.gradingform_matrixeditor.addhandlers()
};

// Adds handlers for clicking submit button. This function must be called each time JS adds new elements to html
M.gradingform_matrixeditor.addhandlers = function() {
    var Y = M.gradingform_matrixeditor.Y
    var name = M.gradingform_matrixeditor.name
    if (M.gradingform_matrixeditor.eventhandler) M.gradingform_matrixeditor.eventhandler.detach()
    M.gradingform_matrixeditor.eventhandler = Y.on('click', M.gradingform_matrixeditor.buttonclick, '#matrix-'+name+' input[type=submit]', null);
}

// switches all input text elements to non-edit mode
M.gradingform_matrixeditor.disablealleditors = function() {
    var Y = M.gradingform_matrixeditor.Y
    var name = M.gradingform_matrixeditor.name
    Y.all('#matrix-'+name+' .level').each( function(node) {M.gradingform_matrixeditor.editmode(node, false)} );
    Y.all('#matrix-'+name+' .description').each( function(node) {M.gradingform_matrixeditor.editmode(node, false)} );
}
...
// function is invoked
M.gradingform_matrixeditor.clickanywhere = function(e) {
    ...
    if (el) {
        if (el.one('textarea').hasClass('hiddenelement')) {
            M.gradingform_matrixeditor.disablealleditors()
            M.gradingform_matrixeditor.editmode(el, true, focustb)
        }
        return
    }
...
M.gradingform_matrixeditor.editmode = function(el, editmode, focustb) {
    ...
    if (!taplain) {
        ...
        taplain = ta.get('parentNode').one('.plainvalue')
        taplain.one('.pseudotablink').on('focus', M.gradingform_matrixeditor.clickanywhere)
        if (tb) {
            ...
            tbplain.one('.pseudotablink').on('focus', M.gradingform_matrixeditor.clickanywhere)
        }
    }
    ...
    if (!editmode) {
        ...
        if (value.length) taplain.removeClass('empty')
            ...
            value = (el.hasClass('level')) ? M.util.get_string('levelempty', 'gradingform_matrix') : M.util.get_string('criterionempty', 'gradingform_matrix')
...
// handler for clicking on submit buttons within matrixeditor element. Adds/deletes/rearranges criteria and/or levels on client side
M.gradingform_matrixeditor.buttonclick = function(e, confirmed) {
    var Y = M.gradingform_matrixeditor.Y
    var name = M.gradingform_matrixeditor.name
    ...
    if (chunks.length>4 || action == 'addlevel') {
        elements_str = '#matrix-'+name+' #'+name+'-criteria-'+chunks[2]+'-levels .level'
    } else {
        elements_str = '#matrix-'+name+' .criterion'
    }
    // prepare the id of the next inserted level or criterion
    var newlevid = 0;
    var newid = 0;
    if (action == 'addcriterion' || action == 'addlevel' || action == 'duplicate' ) {
        newid = M.gradingform_matrixeditor.calculatenewid('#matrix-'+name+' .criterion');
        newlevid = M.gradingform_matrixeditor.calculatenewid('#matrix-'+name+' .level');
    }
    if (chunks.length == 3 && action == 'addcriterion') {
        ...
        for (levidx=0;levidx<levelsscores.length;levidx++) {
            levelsstr += M.gradingform_matrixeditor.templates[name].level.
                replace(/\{LEVEL-id\}/g, 'NEWID'+(newlevid+levidx)).
                replace(/\{LEVEL-score\}/g, levelsscores[levidx]).
                replace(/\{LEVEL-index\}/g, levidx + 1);
        }
        var newcriterion = M.gradingform_matrixeditor.templates[name]['criterion'].replace(/\{LEVELS\}/, levelsstr)
        parentel.append(newcriterion.replace(/\{CRITERION-id\}/g, 'NEWID'+newid).replace(/\{.+?\}/g, ''))
        M.gradingform_matrixeditor.assignclasses('#matrix-'+name+' #'+name+'-criteria-NEWID'+newid+'-levels .level')
        M.gradingform_matrixeditor.addhandlers();
        M.gradingform_matrixeditor.disablealleditors()
        M.gradingform_matrixeditor.assignclasses(elements_str)
        M.gradingform_matrixeditor.editmode(
            Y.one('#matrix-' + name + ' #' + name + '-criteria-NEWID' + newid + '-description-cell'), true
        );
        ...
        var newlevel = M.gradingform_matrixeditor.templates[name]['level'].
        ...
        M.gradingform_matrixeditor.disablealleditors()
        M.gradingform_matrixeditor.assignclasses(elements_str)
        M.gradingform_matrixeditor.editmode(parent.all('.level').item(parent.all('.level').size()-1), true)
        ...
        if (confirmed) {
            ...
            M.util.js_pending('gradingform_matrixeditor:deleteConfirmation');
            require(['core/notification', 'core/str'], function(Notification, Str) {
                Notification.saveCancelPromise(
                    Str.get_string('confirmation', 'admin'),
                    Str.get_string('confirmdeletecriterion', 'gradingform_matrix'),
                    Str.get_string('yes', 'moodle')
                ).then(function() {
                    M.gradingform_matrixeditor.buttonclick.apply(this, [e, true]);
                    return;
                }.bind(this)).catch(function() {
                    // User cancelled.
                });
                M.util.js_complete('gradingform_matrixeditor:deleteConfirmation');
            }.bind(this));
        }
        ...
        var newcriterion = M.gradingform_matrixeditor.templates[name].criterion
        ...
        M.gradingform_matrixeditor.disablealleditors();
        M.gradingform_matrixeditor.assignclasses(elements_str);
        M.gradingform_matrixeditor.editmode(Y.one('#matrix-'+name+' #'+name+'-criteria-NEWID'+newid+'-description-cell'),true);
        ...
        if (confirmed) {
            ...
            M.util.js_pending('gradingform_matrixeditor:deleteLevelConfirmation');
            require(['core/notification', 'core/str'], function(Notification, Str) {
                Notification.saveCancelPromise(
                    Str.get_string('confirmation', 'admin'),
                    Str.get_string('confirmdeletelevel', 'gradingform_matrix'),
                    Str.get_string('yes', 'moodle')
                ).then(function() {
                    M.gradingform_matrixeditor.buttonclick.apply(this, [e, true]);
                    return;
                ...
                M.util.js_complete('gradingform_matrixeditor:deleteLevelConfirmation');
            }.bind(this));
        }
    } else {
        // unknown action
...
M.gradingform_matrixeditor.assignclasses = function (elements_str) {
    var elements = M.gradingform_matrixeditor.Y.all(elements_str)
...
M.gradingform_matrixeditor.calculatenewid = function (elements_str) {
    var newid = 1
    M.gradingform_matrixeditor.Y.all(elements_str).each( function(node) {
